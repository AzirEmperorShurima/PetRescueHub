...
const ResourceLibrary = ({ videos = [] }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [categoryFilter, setCategoryFilter] = useState('');
  const { isOpen: showFilters, onToggle: toggleFilters } = useDisclosure();

  // Early return if videos is not properly loaded
  if (!videos || !Array.isArray(videos)) {
    return (
      <Container maxW="container.xl" py={8} bg={useColorModeValue('gray.50', 'gray.900')} minH="100vh">
  ...
  const categories = useMemo(() => {
    ...
    const uniqueCategories = [...new Set(videos.map(video => video?.category).filter(Boolean))];
                    ...
                    <CardBody p={4}>
                      <VStack align="start" spacing={3}>
                        ...
                        <Badge
                          ...
                          py={1}
                        >
                          {categories.find(cat => cat.value === video?.category)?.label || video?.category || 'Uncategorized'}
                        </Badge>
                      </VStack>
                    </CardBody>
                  </Card>
